  ...
  ({ label, theme = 'dark', checked, ...props }: CheckboxProps, ref) => {
    return (
      <label className="flex items-center cursor-pointer" htmlFor={props.id}>
        ...
        <div
          className={clsx(
            'min-h-[14px] min-w-[14px] border border-alm-white flex items-center justify-center  text-alm-white bg-black focus:outline-none',
            {
              '!bg-white': checked,
            },
          )}
        ...
        <span className="text-alm-white text-[12px] ml-[10px]">{label}</span>
      </label>
    )
  },
)
